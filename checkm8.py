...
def asm_arm64_x7_trampoline(dest):
  return b'\x47\x00\x00\x58\xE0\x00\x1F\xD6' + struct.pack(b'<Q', dest)
...
def prepare_shellcode(name, constants=[]):
  if name.endswith('_armv7'):
    ...
    print(f"ERROR: Shellcode name {name} does not end with known architecture. Exiting.")
    sys.exit(1)

  with open(f"bin/{name}.bin", 'rb') as f:
    shellcode = f.read()
